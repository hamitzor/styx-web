module.exports = {
  'common': [
    'https://ajax.googleapis.com/ajax/libs/jquery/2.1.3/jquery.min.js',
    'https://unpkg.com/popper.js@1.15.0/dist/umd/popper.min.js',
    'https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js',
    'https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.14.15/jquery.mask.js',
    '/public/min.js/paroller.min.js',
    '/public/min.js/globals.min.js',
    '/public/min.js/common.min.js'
  ],
  'anasayfa': [
    'https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.1/jquery.validate.min.js',
    'https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.1/additional-methods.min.js',
    '/public/min.js/paroller.min.js',
    '/public/min.js/slick.min.js',
    '/public/min.js/anasayfa.min.js'
  ],
  'kurumsal': [],
  'iletisim': [
    '/public/min.js/iletisim.min.js',
    '{{src=\'https://maps.googleapis.com/maps/api/js?key=AIzaSyCNRkGfXeUviPEONfF5p3rusAyH_XPH-vQ&callback=contactInitMap\' async defer}}'
  ],
  'not-found': [],
  'urun': [],
  'referanslar': [],
  'teklif-al': [
    'https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.1/jquery.validate.min.js',
    'https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.1/additional-methods.min.js',
    '/public/min.js/teklif-al.min.js'
  ]
}